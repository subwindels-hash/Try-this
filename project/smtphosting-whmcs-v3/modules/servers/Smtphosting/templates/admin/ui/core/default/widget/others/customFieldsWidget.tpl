...
<div class="lu-widget__body">
    <div class="lu-widget__content config-option-box">
        {if $customTplVars['customFields']}
            <div class="lu-row">
                {foreach from=$customTplVars['customFields'] key=customFieldKey item=customFieldItem}
                    <div class="lu-col-md-6 lu-p-r-4x config-option text-left">
                        <b>{$customFieldItem['fieldname']}</b>
                    </div>
                {/foreach}
                {if ($customTplVars['customFields']|count) %2 !== 0}
                    <div class="lu-col-md-6 lu-p-r-4x config-option text-left">
                    </div>
                {/if}
            </div>
