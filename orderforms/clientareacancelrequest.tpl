...
{else}
    ...
    <form method="post" action="{$smarty.server.PHP_SELF}?action=cancel&amp;id={$id}" class="form-stacked">
        ...
        <fieldset>
            ...
            <div class="form-group btn-group-sub-cencel">
                <div class="form-inline">
                    <input type="submit" value="{$LANG.clientareacancelrequestbutton}" class="btn btn-danger" />
                    <a class="cencel-btn-bottom" href="clientarea.php?action=productdetails&id={$id}" class="btn btn-default">{$LANG.cancel}</a>
                </div>
            </div>
            </div>
